...
const Mint: React.FC<React.PropsWithChildren> = () => {
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {t('You can change your profile pic later if you get another approved GroFi DEX Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% U2U', { num: formatUnits(MINT_COST, 18) })}
          </Text>
          {starterNfts.map((nft) => {
          ...
          {!hasMinimumCakeRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% U2U is required', { num: formatUnits(MINT_COST, 18) })}
            </Text>
          )}
