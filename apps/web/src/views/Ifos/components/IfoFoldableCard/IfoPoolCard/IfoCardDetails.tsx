...
const MaxTokenEntry = ({
  ...
  const basicTooltipContent =
    ifo.version >= 3.1 && !basicSale
      ...
      : t(
          'For the basic sale, Max U2U entry is capped by minimum between your average U2U balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake more U2U into the iCAKE',
        )
  ...
  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      ...
      t(
        'For the unlimited sale, Max U2U entry is capped by your average U2U balance in the iCake. To increase the max entry, Stake more U2U into the iCake',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. U2U entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('U2U to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
